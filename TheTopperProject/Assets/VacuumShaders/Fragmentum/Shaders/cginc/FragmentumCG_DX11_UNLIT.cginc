...
v2f vertS (v_data v)
{
    v2f o;
		...
		float3 dir = buf_Normal[index];	
			...
			dir = mul((float3x3)unity_WorldToObject, V_FR_DisplaceDirectionObjectPosition.xyz);
			dir = normalize(dir);
		#endif
...
void geomSE(triangle GS_INPUT input[3], inout TriangleStream<FS_INPUT> triStream)
{
	...
	float4 p[6];
	p[0] = UnityObjectToClipPos(input[0].pos);
	p[1] = UnityObjectToClipPos(input[1].pos);
	p[2] = UnityObjectToClipPos(input[2].pos);

	p[3] = UnityObjectToClipPos((input[0].pos + float4(dirV, 0) * input[0].info.x));
	p[4] = UnityObjectToClipPos((input[1].pos + float4(dirV, 0) * input[1].info.x));
	p[5] = UnityObjectToClipPos((input[2].pos + float4(dirV, 0) * input[2].info.x));
...
{ 	
	...
	#ifdef LIGHTMAP_ON
		fixed3 lm = ( FR_DecodeLightmap (UNITY_SAMPLE_TEX2D(unity_Lightmap, i.uvLM)));
		tex.rgb *= lm;
	#endif
