...
{ 
		...
		half fragmentFactor = 1;
			...
			half4 worldPos = mul(unity_ObjectToWorld, half4(v.tangent.xyz, 1));
			...
			half3 worldPos = mul(unity_ObjectToWorld, half4(v.tangent.xyz, 1)).xyz;
		...
		half3 dir = cNormal;
			...
			dir = mul((half3x3)unity_WorldToObject, V_FR_DisplaceDirectionObjectPosition.xyz);
			dir = normalize(dir);
		#endif
		#if defined(V_FR_DISPLACE_RADIAL)
			half3 displaceObjectPosition = mul(unity_WorldToObject, half4(V_FR_DisplaceDirectionObjectPosition.xyz, 1)).xyz * 1.0;
		...
		//Rotate
			...
			v.vertex.xyz = V_FR_RotateArbitrary(v.vertex, mul(unity_WorldToObject, half4(V_FR_RotateCustomPointPosition.xyz, 1)).xyz * 1.0, mul(unity_WorldToObject, half4(V_FR_RotateCustomPointNormal.xyz, 0)).xyz, theta);
		...
		#ifdef V_FR_REFLECTION
			half3 viewDir = _WorldSpaceCameraPos.xyz - mul(unity_ObjectToWorld, v.vertex).xyz;
			half3 worldN = mul((half3x3)unity_ObjectToWorld, vNormal * 1.0);
			...
			#ifdef V_FR_CALC_LIGHT_PER_VERTEX
				half3 normal = mul((half3x3)unity_ObjectToWorld, vNormal * 1.0);
				o.color = fixed4(max(0, dot (normalize(normal), _WorldSpaceLightPos0.xyz)) * _LightColor0.rgb, 0);
			#else
				o.normal = mul((half3x3)unity_ObjectToWorld, vNormal * 1.0);
			...
			#ifdef V_FR_CALC_LIGHT_PER_VERTEX
				half3 normal = mul((half3x3)unity_ObjectToWorld, vNormal * 1.0);
				o.color = fixed4(max(0, dot (normalize(normal), (normalize(_WorldSpaceLightPos0.xyz - mul(unity_ObjectToWorld, v.vertex).xyz)))) * _LightColor0.rgb, 0);
			#else
				o.normal = mul((half3x3)unity_ObjectToWorld, vNormal * 1.0);
				o.lightDir = _WorldSpaceLightPos0.xyz - mul(unity_ObjectToWorld, v.vertex).xyz;
			#endif
		#endif 
		...
		#ifdef V_FR_REFLECTION
			half3 viewDir = _WorldSpaceCameraPos.xyz - mul(unity_ObjectToWorld, v.vertex).xyz;
			half3 worldN = mul((half3x3)unity_ObjectToWorld, v.normal * 1.0);
			o.refl = reflect( -viewDir, worldN );
		#endif

		#ifdef PASS_FORWARD_BASE
			#ifdef V_FR_CALC_LIGHT_PER_VERTEX
				half3 normal = mul((half3x3)unity_ObjectToWorld, v.normal * 1.0);
				o.color = fixed4(max(0, dot (normalize(normal), _WorldSpaceLightPos0.xyz)) * _LightColor0.rgb, 0);
			#else
				o.normal = mul((half3x3)unity_ObjectToWorld, v.normal * 1.0);
				o.lightDir = _WorldSpaceLightPos0.xyz;
			#endif
		#endif
